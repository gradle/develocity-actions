 ...
 */
async function installDevelocityAgent(
    agentUrlOverride: string,
    version: string,
    develocityAgentInstallLocation: string
): Promise<void> {
...
function configureEnvironment(wrappersDir: string): void {
    ...
    if (develocityAccessKey) {
        if (!develocityUrl) {
            core.warning(
                'develocity-access-key was provided but develocity-server-url was not. The access key will not be set.'
            )
        } else {
...
async function findAndVerifyBinary(binaryName: string): Promise<string> {
    ...
    try {
        ...
        throw new Error(
            `Found ${binaryName} at ${actualBinary} but it's not executable or not a valid ${binaryName} binary`
        )
    }
